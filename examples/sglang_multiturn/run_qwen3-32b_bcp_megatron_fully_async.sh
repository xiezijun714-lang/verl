...
#
# Run from project root: bash examples/sglang_multiturn/run_qwen3-32b_bcp_megatron_fully_async.sh
...
mkdir -p "$LOG_DIR"
LOG_FILE="${BCP_LOG_FILE:-${LOG_DIR}/megatron_fully_async_run.log}"
...
echo "[config] entry=fully_async, trainer GPUs=${TRAINER_TOTAL_GPUS}, rollout GPUs=${ROLLOUT_TOTAL_GPUS}"
...
echo "[train] Starting fully async trainer: trigger_sync=${FULL_ASYNC_TRIGGER_PARAMETER_SYNC_STEP}, staleness=${FULL_ASYNC_STALENESS_THRESHOLD}, partial=${FULL_ASYNC_PARTIAL_ROLLOUT}, optim_steps=${FULL_ASYNC_TOTAL_TRAINING_STEPS}"
"$PYTHON_BIN" -m verl.experimental.fully_async_policy.fully_async_main \
    ...
    trainer.experiment_name="${EXPERIMENT_NAME:-qwen3-32b_bcp_megatron-fully-async-32k-grpo}" \
    trainer.default_local_dir="${CKPT_DIR:-${PROJECT_DIR}/ckpt}/${EXPERIMENT_NAME:-qwen3-32b_bcp_megatron-fully-async-32k-grpo}" \
