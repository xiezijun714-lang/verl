#!/bin/bash
# Multi-turn Search Tool-Calling GRPO on BrowseComp-Plus, 4 nodes, Qwen3-8B, SGLang rollout
# Run from project root on head node: bash examples/sglang_multiturn/run_qwen3-8b_bcp_fsdp.sh
...
mkdir -p "$LOG_DIR"
LOG_FILE="${LOG_DIR}/fsdp_run.log"
exec > >(tee -a "$LOG_FILE") 2>&1


VENV_PATH="/root/paddlejob/workspace/env_run/xzj/venv_echo"
HEAD_IP="10.63.234.28"
WORKER_IPS="10.63.234.146 10.63.234.19 10.63.234.143"

source "${VENV_PATH}/bin/activate"

# 修复 activate 脚本中错误的 VIRTUAL_ENV 路径
...
PROJECT_DIR="$(pwd)"
MODEL_PATH="/root/paddlejob/workspace/env_run/xzj/models/Qwen3-8B"
...
for ip in $WORKER_IPS; do
    ...
    ssh "$ip" "export SGLANG_ALLOW_OVERWRITE_LONGER_CONTEXT_LEN=1 && source ${VENV_PATH}/bin/activate && ray start --address='${HEAD_IP}:6379' --num-gpus=8"
done
...
echo "[retriever] Waiting for server ready (first run ~15min; cached ~30s) ..."
...
python3 -m verl.trainer.main_ppo \
    ...
    actor_rollout_ref.rollout.multi_turn.max_assistant_turns=null \
    ...
    trainer.experiment_name='qwen3-8b_bcp_fsdp_32k-vanilla' \
