#!/bin/bash
# Multi-turn CodeGym Tool-Calling GRPO baseline
# 4 nodes x 8 H100-80GB, Qwen3-32B, Megatron backend, SGLang rollout
#
# Run from project root:
#   bash examples/sglang_multiturn/run_qwen3-32b_codegym_megatron_grpo.sh
...
mkdir -p "$LOG_DIR"
LOG_FILE="${LOG_DIR}/megatron_codegym_grpo_run.log"
...
VALIDATION_DATA_DIR="${VALIDATION_DATA_DIR:-${PROJECT_DIR}/val_outputs_codegym_grpo}"
...
echo "[config] CodeGym GRPO vanilla context: prompt=${MAX_PROMPT_LENGTH}, response=${MAX_RESPONSE_LENGTH}, max_model=${MAX_MODEL_LEN}"
...
echo "[codegym] Sampling ${TRAIN_MAX_SAMPLES} training examples from raw data ..."
"${VENV_PATH}/bin/python3" - <<PY
import ast
import pathlib
import pandas as pd

out_dir = pathlib.Path("${FILTERED_DATA_DIR}")
out_dir.mkdir(parents=True, exist_ok=True)

raw_files = ast.literal_eval("""${TRAIN_FILES_RAW}""")
dfs = [pd.read_parquet(f) for f in raw_files]
df = pd.concat(dfs, ignore_index=True)
print(f"total raw train rows: {len(df)}")

sampled = df.sample(n=min(${TRAIN_MAX_SAMPLES}, len(df)), random_state=42).reset_index(drop=True)
dst = pathlib.Path("${TRAIN_SAMPLED}")
sampled.to_parquet(dst, index=False)
print(f"wrote {dst}: rows={len(sampled)}")
PY

if [ ! -f "$VAL_FILES" ]; then
    echo "[codegym] ERROR: expected validation file not found: $VAL_FILES"
    echo "[codegym] Build it separately with examples/data_preprocess/codegym_qwen3_filter_val.py select from a rollout-n=4 dump using --threshold 0.25 --expected-repeats 4."
    exit 1
fi
...
for i in $(seq 1 1200); do
    ...
    if [ -n "$INSTANCE_JSON" ]; then
        INSTANCE_UID=$(python3 -c "import json,sys; print(json.loads(sys.argv[1]).get('uid',''))" "$INSTANCE_JSON" 2>/dev/null || true)
        if [ -n "$INSTANCE_UID" ]; then
            curl --noproxy '*' -sf "${CODEGYM_SERVER_HOST}/release_instance?uid=${INSTANCE_UID}" >/dev/null 2>&1 || true
...
python3 -m verl.trainer.main_ppo \
    ...
    trainer.experiment_name='qwen3-32b_codegym_megatron-grpo-32k-vanilla' \
    trainer.default_local_dir="${CKPT_DIR:-${PROJECT_DIR}/ckpt}/qwen3-32b_codegym_megatron-grpo-32k-vanilla" \
