...
for ip in $WORKER_IPS; do
    timeout 10 scp -o ConnectTimeout=5 "$SCRIPT_FILE" "$ip":"$SCRIPT_FILE" || echo "[sync] WARNING: failed to sync script to $ip"
done
echo "[sync] Script sync done."

echo "[sync] Syncing tool_agent_loop.py to worker nodes ..."
TOOL_AGENT_LOOP_FILE="${PROJECT_DIR}/verl/experimental/agent_loop/tool_agent_loop.py"
for ip in $WORKER_IPS; do
    timeout 10 scp -o ConnectTimeout=5 "$TOOL_AGENT_LOOP_FILE" "$ip":"$TOOL_AGENT_LOOP_FILE" || echo "[sync] WARNING: failed to sync tool_agent_loop.py to $ip"
done
echo "[sync] Python files sync done."
